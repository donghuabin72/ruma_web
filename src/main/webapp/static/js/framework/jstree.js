...
(function(factory) {
	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($, undefined) {
			 ...
			 * actual class and methods to create, access and manipulate
			 * instances.
			 * 
			 * @name $.jstree
			 */
			...
			/**
			 * remove all traces of jstree from the DOM and destroy all
			 * instances
			 * 
			 * @name $.jstree.destroy()
			 */
			 ...
			 * the same instance $.jstree.reference('tree');
			 * $.jstree.reference('#tree'); $.jstree.reference($('#tree'));
			 * $.jstree.reference(document.getElementByID('tree'));
			 * $.jstree.reference('branch'); $.jstree.reference('#branch');
			 ...
			 * one is created and `arg` is used to extend `$.jstree.defaults`
			 ...
			 * __Examples__
			 * 
			 * $('#tree1').jstree(); // creates an instance $('#tree2').jstree({
			 * plugins : [] }); // create an instance with some options
			 * $('#tree1').jstree('open_node', '#branch_1'); // call a method on
			 * an existing instance, passing additional arguments
			 * $('#tree2').jstree(); // get an existing instance (or create an
			 * instance) $('#tree2').jstree(true); // get an existing instance
			 ...
			 * __Examples__
			 * 
			 * $('div:jstree').each(function () { $(this).jstree('destroy'); });
			 * 
			 * @name $(':jstree')
			 * @return {jQuery}
			 */
			...
			$.jstree.defaults.core = {
				 ...
				 * and jstree will automatically determine if the response is
				 ...
				 * 
				 * __Examples__ // AJAX $('#tree').jstree({ 'core' : { 'data' : {
				 * 'url' : '/get/children/', 'data' : function (node) { return {
				 * 'id' : node.id }; } } }); // direct data $('#tree').jstree({
				 * 'core' : { 'data' : [ 'Simple root node', { 'id' : 'node_2',
				 * 'text' : 'Root node with options', 'state' : { 'opened' :
				 * true, 'selected' : true }, 'children' : [ { 'text' : 'Child
				 * 1' }, 'Child 2'] } ] }); // function $('#tree').jstree({
				 ...
				 * __Examples__
				 * 
				 * $('#tree').jstree({ 'core' : { 'strings' : { 'Loading ...' :
				 * 'Please wait ...' } } });
				 * 
				 ...
				 * __Examples__
				 * 
				 * $('#tree').jstree({ 'core' : { 'check_callback' : function
				...
				themes : {
					...
					url : false,
					/**
					 * the location of all jstree themes - only used if `url` is
					 * set to `true`
					 * 
			...
			$.jstree.core.prototype = {
				...
				init : function(el, options) {
					...
					this.element
							.html("<"
									...
									+ "a class='jstree-anchor' href='#'><i class='jstree-icon jstree-themeicon-hidden'></i>" + this.get_string("Loading ...")
									+ "</a></li></ul>");
				 ...
				 * 
				 * @name destroy()
				 * @param {Boolean} keep_html if not set to `true` the container
				 *            will be emptied, otherwise the current DOM
				 *            elements will be kept intact
				 */
				...
				bind : function() {
					...
					this.element.on("dblclick.jstree", function(e) {
						...
						e.preventDefault(); // prevent any node inside from
						...
						case 40: // down
							...
							break;
						case 106: // aria defines * on numpad as open_all -
							// not very common
							this.open_all();
							break;
						case 36: // home
						 ...
						 * !== '#') { o = this.is_selected(o) ?
						 * this.get_selected() : o; this.delete_node(o); }
						 * break; // f2 case 113: e.preventDefault(); o =
						 * this.get_node(e.currentTarget); if(o && o.id && o.id
						 * !== '#') { // this.edit(o); } break; default: //
						 * console.log(e.which); break;
						 */
						}
					...
					.on(
							...
							$.proxy(function(e) {
								...
								var chr = String.fromCharCode(e.which).toLowerCase(), col = this.element.find('.jstree-anchor').filter(':visible'), ind = col
										.index(document.activeElement) || 0, end = false;
								word += chr;

								// match for whole word from current node down
								...
								if (word.length > 1) {
									...
									// match for whole word from the beginning
									// of the tree
								...
								// list nodes that start with that letter (only
								// if word consists of a single char)
								if (new RegExp('^' + chr.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + '+$').test(word)) {
									// search for the next node starting with
									// that letter
				...
				get_node : function(obj, as_dom) {
					...
					try {
						...
						} else if (typeof obj === "string" && (dom = $('#' + obj.replace($.jstree.idregex, '\\$&'), this.element)).length
				 ...
				 * @param {String} glue if you want the path as a string - pass
				 *            the glue here (for example '/'), if a falsy value
				 *            is supplied here, an array is returned
				 ...
				 *            loading is complete, the function is executed in
				 *            the instance's scope and receives two arguments -
				...
				load_node : function(obj, callback) {
					...
					if (obj.state.loaded) {
						...
						for (k = 0, l = obj.children_d.length; k < l; k++) {
							for (i = 0, j = obj.parents.length; i < j; i++) {
								this._model.data[obj.parents[i]].children_d = $.vakata.array_remove_item(this._model.data[obj.parents[i]].children_d,
										obj.children_d[k]);
							}
				 ...
				 * @param {function} callback a function to be executed once
				 *            loading is complete, the function is executed in
				 *            the instance's scope and receives one argument -
				 *            the array passed to _load_nodes
				 */
				 ...
				 * @param {mixed} obj the node to load recursively, omit to load
				 ...
				 * @param {function} callback a function to be executed once
				 *            loading is complete, the function is executed in
				 *            the instance's scope and receives one argument - a
				 *            boolean status
				 * @return {Boolean}
				 */
				_load_node : function(obj, callback) {
					...
					if ($.isFunction(s)) {
						return s.call(this, obj, $.proxy(function(d) {
							...
							// '_append_html_data' : '_append_json_data'](obj,
							// typeof d === 'string' ? $(d) : d));
						}, this));
					}
					if (typeof s === 'object') {
				 ...
				 */
				_redraw : function() {
					var nodes = this._model.force_full_redraw ? this._model.data['#'].children.concat([]) : this._model.changed.concat([]), f = document
				...
				redraw_node : function(node, deep, is_callback, force_render) {
					var obj = this.get_node(node), par = false, ind = false, old = false, i = false, j = false, k = false, c = '', d = document, m = this._model.data, f = false, s = false, tmp = null, t = 0, l = 0;
					if (!obj) {
						return false;
					}
					if (obj.id === '#') {
					...
					node = !document.querySelector ? document.getElementById(obj.id) : this.element[0].querySelector('#'
							+ ("0123456789".indexOf(obj.id[0]) !== -1 ? '\\3' + obj.id[0] + ' ' + obj.id.substr(1).replace($.jstree.idregex, '\\$&') : obj.id
				 ...
				 *            milliseconds when opening the node (overrides the
					...
					if (!this.is_loaded(obj)) {
						...
						if (d.length) {
							...
							if (!animation) {
								...
								d.children(".jstree-children").css("display", "none").end().removeClass("jstree-closed").addClass("jstree-open").attr(
										"aria-expanded", true).children(".jstree-children").stop(true, true).slideDown(animation, function() {
									this.style.display = "";
				 ...
				 * @param {Number} animation the animation duration in
				 *            milliseconds when closing the node (overrides the
				 *            `core.animation` setting). Use `false` for no
				 *            animation.
				...
				close_node : function(obj, animation) {
					...
					if (d.length) {
						if (!animation) {
							...
							d.children(".jstree-children").attr("style", "display:block !important").end().removeClass("jstree-open").addClass("jstree-closed")
									.attr("aria-expanded", false).children(".jstree-children").stop(true, true).slideUp(animation, function() {
										this.style.display = "";
				 ...
				 * @param {mixed} obj the node to open recursively, omit to open
				 *            all nodes in the tree
				 * @param {Number} animation the animation duration in
				 *            milliseconds when opening the nodes, the default
				 *            is no animation
				 ...
				 * @param {Number} animation the animation duration in
				 *            milliseconds when closing the nodes, the default
				 *            is no animation
				...
				close_all : function(obj, animation) {
					...
					var dom = obj.id === '#' ? this.get_container_ul() : this.get_node(obj, true), _this = this, i, j;
					if (!dom.length) {
						for (i = 0, j = obj.children_d.length; i < j; i++) {
				 ...
				 * @private
				 * @param {Object} state the state to restore. Keep in mind this
				 *            object is passed by reference and jstree will
				 *            modify it.
				 ...
				 *            be reapplied, if set to a function (receiving the
				...
				refresh : function(skip_loading, forget_state) {
					...
					if (!skip_loading) {
						this.element.html("<" + "ul class='" + c + "' role='group'><"
								+ "li class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='treeitem' id='j" + this._id
				...
				refresh_node : function(obj) {
					...
					this._load_nodes(to_load, $.proxy(function(nodes) {
						 ...
						 * @param {Array} nodes - an array of the IDs of the
						 *            nodes that were reloaded
						 */
				 ...
				 * @param {mixed} obj the node, you can pass an array to set the
				 ...
				 * @param {mixed} par the parent node (to create a root node use
				 ...
				 *            "first" and "last" are also supported, default is
				 *            "last"
				...
				create_node : function(par, node, pos, callback, is_loaded) {
					 ...
					 * @param {Number} position the position of the new node
				...
				check : function(chk, obj, par, pos, more) {
					...
					if (chk === "move_node" || chk === "copy_node") {
						if ((!more || !more.is_multi)
								&& (obj.id === par.id || $.inArray(obj.id, par.children) === pos || $.inArray(par.id, obj.children_d) !== -1)) {
							this._data.core.last_error = {
								'error' : 'check',
								'plugin' : 'core',
								'id' : 'core_01',
				 ...
				 *            is completed, receives 3 arguments - the node, the
				 *            new parent and the position
				 * @param {Boolean} is_loaded internal parameter indicating if
				 *            the parent node has been loaded
				 * @param {Boolean} skip_redraw internal parameter indicating if
				 *            the tree should be redrawn
				...
				move_node : function(obj, par, pos, callback, is_loaded, skip_redraw, origin) {
					 ...
					 * @param {Object} node
					 * @param {String} parent the parent's ID
					 * @param {Number} position the position of the node among
					 *            the parent's children
					 * @param {String} old_parent the old parent of the node
					 * @param {Number} old_position the old position of the node
					 * @param {Boolean} is_multi do the node and new parent
					 *            belong to different instances
					 * @param {jsTree} old_instance the instance the node came
					 *            from
				...
				copy_node : function(obj, par, pos, callback, is_loaded, skip_redraw, origin) {
					 ...
					 *            the parent's children
					 * @param {String} old_parent the old parent of the node
					 * @param {Number} old_position the position of the original
					 *            node
					 * @param {Boolean} is_multi do the node and new parent
					 *            belong to different instances
					 * @param {jsTree} old_instance the instance the node came
					...
					this.trigger('copy_node', {
						...
						"position" : pos,
						"old_parent" : old_par,
						"old_position" : old_ins && old_ins._id && old_par && old_ins._model.data[old_par] && old_ins._model.data[old_par].children ? $
				 ...
				 *            integer, "first" and "last" are supported),
				 ...
				 *            with (if omitted or set to a non-string value the
				 *            node's text value is used)
				 * @param {Function} callback a function to be called once the
				 *            text box is blurred, it is called in the
				 *            instance's scope and receives the node and a
				 *            status parameter - true if the rename is
				 *            successful, false otherwise. You can access the
				...
				edit : function(obj, default_text, callback) {
					...
					h2 = $("<" + "input />", {
						...
						"blur" : $.proxy(function() {
							...
							if (!nv) {
								this.set_text(obj, t); // move this up? and fix
								// #483
							}
							if (callback) {
				 ...
				 *            this theme. Omit or set to `false` if you manually
				 ...
				 * @param {mixed} obj
				 * @param {String} icon the new icon - can be a path to an icon
				 *            or a className, if using an image that is in the
			...
			$.jstree.plugins.checkbox = function(options, parent) {
																...
																for (k = 0, l = chd.length; k < l; k++) {
																	...
																	while (p && p.id !== '#') {
																		...
																		if (c === j) {
																			...
																			if (tmp && tmp.length) {
																				tmp.attr('aria-selected', true).children('.jstree-anchor').addClass(
												...
												.proxy(
														function(e, data) {
															var obj = data.node, m = this._model.data, par = this.get_node(obj.parent), dom = this.get_node(
																	obj, true), i, j, c, tmp, s = this.settings.checkbox.cascade, t = this.settings.checkbox.tie_selection;
															...
															if (s.indexOf('down') !== -1 && dom.length) {
																dom.find('.jstree-anchor').addClass(t ? 'jstree-clicked' : 'jstree-checked').parent().attr(
																		'aria-selected', true);
															}
														}, this))
								.on(this.settings.checkbox.tie_selection ? 'deselect_all.jstree' : 'uncheck_all.jstree', $.proxy(function(e, data) {
									var obj = this.get_node('#'), m = this._model.data, i, j, tmp;
									for (i = 0, j = obj.children_d.length; i < j; i++) {
										tmp = m[obj.children_d[i]];
										if (tmp && tmp.original && tmp.original.state && tmp.original.state.undetermined) {
											tmp.original.state.undetermined = false;
														...
														function(e, data) {
															...
															if (s.indexOf('up') !== -1) {
																for (i = 0, j = obj.parents.length; i < j; i++) {
																	...
																	if (tmp && tmp.length) {
																		tmp.attr('aria-selected', false).children('.jstree-anchor').removeClass(
																				t ? 'jstree-clicked' : 'jstree-checked');
																	}
																}
															}
															...
															for (i = 0, j = this._data[t ? 'core' : 'checkbox'].selected.length; i < j; i++) {
																// apply down +
																// apply up
																if ((s.indexOf('down') === -1 || $.inArray(this._data[t ? 'core' : 'checkbox'].selected[i],
																		obj.children_d) === -1)
																		&& (s.indexOf('up') === -1 || $.inArray(
																				this._data[t ? 'core' : 'checkbox'].selected[i], obj.parents) === -1)) {
															...
															// separately?)
															if (s.indexOf('down') !== -1 && dom.length) {
																dom.find('.jstree-anchor').removeClass(t ? 'jstree-clicked' : 'jstree-checked').parent().attr(
																		'aria-selected', false);
															}
														}, this));
					}
				...
				this._undetermined = function() {
					...
					this.element.find('.jstree-closed').not(':has(.jstree-children)').each(
							function() {
				 ...
				 * 
				 * @name check_node(obj)
				 * @param {mixed} obj an array can be used to check multiple
				 *            nodes
			...
			$.jstree.defaults.contextmenu = {
				 ...
				 * and action are required): * `separator_before` - a boolean
				 * indicating if there should be a separator before this item *
				 * `separator_after` - a boolean indicating if there should be a
				 * separator after this item * `_disabled` - a boolean
				 * indicating if this action should be disabled * `label` - a
				 * string - the name of the action (could be a function
				 * returning a string) * `action` - a function to be executed if
				 * this item is chosen * `icon` - a string, can be a path to an
				 * icon or a className, if using an image that is in the current
			...
			$.jstree.plugins.contextmenu = function(options, parent) {
				this.bind = function() {
					...
					/*
					 * if(!('oncontextmenu' in document.body) && ('ontouchstart'
					 * in document.body)) { var el = null, tm = null;
					 * this.element .on("touchstart", ".jstree-anchor", function
					 * (e) { el = e.currentTarget; tm = +new Date();
				 ...
				 * @param {mixed} obj the node
				 * @param {Number} x the x-coordinate relative to the document
				 *            to show the menu at
				 * @param {Number} y the y-coordinate relative to the document
				 *            to show the menu at
				...
				this._show_contextmenu = function(obj, x, y, i) {
					 ...
					 * @param {Number} x the x-coordinate of the menu relative
					 *            to the document
					 * @param {Number} y the y-coordinate of the menu relative
					 *            to the document
					 * @plugin contextmenu
					 */
			...
			(function($) {
				...
				$.vakata.context = {
						...
						$.each(o, function(i, val) {
							...
							str += ($.isFunction(val.label) ? val.label({
								...
								"element" : vakata_context.element
									...
									+ (val.shortcut ? ' <span class="vakata-contextmenu-shortcut vakata-contextmenu-shortcut-' + val.shortcut + '">'
					...
					_show_submenu : function(o) {
						...
						var e = o.children("ul"), x = o.offset().left + o.outerWidth(), y = o.offset().top, w = e.width(), h = e.height(), dw = $(window)
								.width()
					...
					show : function(reference, position, data) {
						...
						if (vakata_context.items.length) {
							 ...
							 * @param {jQuery} reference the element that was
							 *            right clicked
							 * @param {jQuery} element the DOM element of the
							 *            menu itself
							 * @param {Object} position the x & y coordinates of
							 *            the menu
							 */
											...
											.proxy(
													function(e) {
														...
														if (e.type === "touchstart"
																&& (!this.settings.dnd.touch || (this.settings.dnd.touch === 'selected' && !$(e.currentTarget)
														...
														var obj = this.get_node(e.target), mlt = this.is_selected(obj) && this.settings.dnd.drag_selection ? this
																...
																: 1, txt = (mlt > 1 ? mlt + ' ' + this.get_string('nodes') : this.get_text(e.currentTarget));
														if (this.settings.core.force_text) {
														...
														if (obj
															...
															}, '<div id="jstree-dnd" class="jstree-' + this.get_theme() + ' jstree-' + this.get_theme() + '-'
																	+ this.get_theme_variant() + ' '
																	+ (this.settings.core.themes.responsive ? ' jstree-dnd-responsive' : '')
																	+ '"><i class="jstree-icon jstree-er"></i>' + txt
								...
								function(e, data) {
									...
									if (ins && ins._data && ins._data.dnd) {
										...
										data.helper.children().attr(
												'class',
												'jstree-' + ins.get_theme() + ' jstree-' + ins.get_theme() + '-' + ins.get_theme_variant() + ' '
														+ (ins.settings.core.themes.responsive ? ' jstree-dnd-responsive' : '')).find('.jstree-copy').first()[data.data.origin
										...
										if ((data.event.target === ins.element[0] || data.event.target === ins.get_container_ul()[0])
														...
														&& ins
																.check(
																		...
																		(data.data.origin && data.data.origin !== ins ? data.data.origin
																				.get_node(data.data.nodes[t1]) : data.data.nodes[t1]), '#', 'last', {
																			'dnd' : true,
																			'ref' : ins.get_node('#'),
																			'pos' : 'i',
											...
											ref = ins.settings.dnd.large_drop_target ? $(data.event.target).closest('.jstree-node').children('.jstree-anchor')
				...
				$.vakata.dnd = {
					...
					_scroll : function(init_only) {
						...
						if (i !== vakata_dnd.scroll_e.scrollTop() || j !== vakata_dnd.scroll_e.scrollLeft()) {
							 ...
							 * @param {Mixed} data any data supplied with the
							 *            call to $.vakata.dnd.start
							 * @param {DOM} element the DOM element being
							 *            dragged
							 * @param {jQuery} helper the helper shown next to
							 *            the mouse
							 * @param {jQuery} event the element that is
							 *            scrolling
							 */
					...
					drag : function(e) {
						...
						if (!vakata_dnd.is_drag) {
							if (Math.abs(e.pageX - vakata_dnd.init_x) > (vakata_dnd.is_touch ? $.vakata.dnd.settings.threshold_touch
								 ...
								 * @param {Mixed} data any data supplied with
								 *            the call to $.vakata.dnd.start
								 * @param {DOM} element the DOM element being
								 *            dragged
								 * @param {jQuery} helper the helper shown next
								 *            to the mouse
								 * @param {Object} event the event that caused
								 *            the start (probably mousemove)
								 */
						 ...
						 *            mouse
						 * @param {Object} event the event that caused this to
			 ...
			 * ### Massload plugin
			 * 
			 * Adds massload functionality to jsTree, so that multiple nodes can
			 * be loaded in a single request (only useful with lazy loading).
			 */
			...
			$.jstree.plugins.massload = function(options, parent) {
				...
				this._load_node = function(obj, callback) {
					...
					if (d) {
						return this[typeof d === 'string' ? '_append_html_data' : '_append_json_data'](obj, typeof d === 'string' ? $($.parseHTML(d)).filter(
								function() {
									return this.nodeType !== 3;
								}) : d, function(status) {
							callback.call(this, status);
			...
			$.jstree.plugins.search = function(options, parent) {
					...
					this.element.on(
							...
							$.proxy(function(e, data) {
								...
								if (r && r.length) {
											...
											+ $.map(
													...
													function(v) {
														return "0123456789".indexOf(v[0]) !== -1 ? '\\3' + v[0] + ' '
																+ v.substr(1).replace($.jstree.idregex, '\\$&') : v.replace($.jstree.idregex, '\\$&');
													}).join(', #')));
									...
									if (this._data.search.som && this._data.search.res.length) {
										...
										o = s.length ? $(this.element[0].querySelectorAll('#'
												+ $.map(
														s,
														function(v) {
															return "0123456789".indexOf(v[0]) !== -1 ? '\\3' + v[0] + ' '
						...
						if (this._data.search.som) {
							if (data.nodes.length) {
								...
								data.nodes.parentsUntil(".jstree").addBack().show().filter(".jstree-children").each(function() {
									$(this).children(".jstree-node:visible").eq(-1).addClass("jstree-last");
								});
							}
						}
				 ...
				 *            matching nodes will be shown (keep in mind this
				 *            can be very slow on large trees or old browsers)
				...
				this.search = function(str, skip_async, show_only_matches, inside, append, show_only_matches_children) {
					...
					$.each(m[inside ? inside : '#'].children_d, function(ii, i) {
						var v = m[i];
						if (v.text && ((s.search_callback && s.search_callback.call(this, str, v)) || (!s.search_callback && f.search(v.text).isMatch))
					 ...
					 * @event
					 * @name search.jstree
					 * @param {jQuery} nodes a jQuery collection of matching
					 *            nodes
					 * @param {String} str the search string
				...
				this.clear_search = function() {
					 ...
					 * @param {jQuery} nodes a jQuery collection of matching
					 *            nodes (the result from the last search)
					 * @param {String} str the search string (the last search
					 *            string)
					 * @param {Array} res a collection of objects represeing the
			...
			$.jstree.plugins.sort = function(options, parent) {
				 ...
				 * @name sort(obj [, deep])
				 * @param {mixed} obj the node
				 * @param {Boolean} deep if set to `true` nodes are sorted
				 *            recursively.
				 * @plugin sort
				 * @trigger search.jstree
				 */
			...
			$.jstree.defaults.state = {
				 ...
				 * Defaults to `changed.jstree open_node.jstree
				 * close_node.jstree`.
				 * 
			...
			$.jstree.plugins.types = function(options, parent) {
				 ...
				 * @param {mixed} obj the node to find the rules for
				 * @param {Boolean} rules if set to `true` instead of a string
				 *            the settings object will be returned
			...
			$.jstree.plugins.wholerow = function(options, parent) {
				this.bind = function() {
					...
					this.element.on('ready.jstree set_state.jstree', $.proxy(function() {
						...
						this.get_node(data.node, true).find('.jstree-clicked').parent().children('.jstree-wholerow').addClass('jstree-wholerow-clicked');
							...
							$.proxy(function(e, data) {
								...
								this.get_node(data.node, true).children('.jstree-wholerow')[e.type === "hover_node" ? "addClass" : "removeClass"]
					 ...
					 * ".jstree-wholerow", function (e) { if(e.target ===
