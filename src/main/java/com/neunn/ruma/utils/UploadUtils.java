...
public class UploadUtils {
     ...
     * 
     * @param request
     * @return infos info[0] 验证文件域返回错误信息 info[1] 上传文件错误信息 info[2] savePath info[3] saveUrl info[4]
     *         fileUrl
     */
    ...
    private String saveFile(FileItem item) {
        ...
        if (item.getSize() > maxSize) { // 检查文件大小
            ...
            try {
                ...
                /*
                 * FileOutputStream fos = new FileOutputStream(uploadFile); // 文件全在内存中 if
                 * (item.isInMemory()) { fos.write(item.get()); } else { InputStream is =
                 * item.getInputStream(); byte[] buffer = new byte[1024]; int len; while ((len =
                 * is.read(buffer)) > 0) { fos.write(buffer, 0, len); } is.close(); } fos.close();
                 * item.delete();
                 */
            } catch (IOException e) {
                e.printStackTrace();
