...
//    }
     ...
     * @since 1.0
     */
    public static void init(Resource xssPath) {
        logger.info("XSSSecurityManager init(FilterConfig config) begin");
        // 初始化安全过滤配置  
        try {
            if (initConfig(xssPath)) {
            ...
            logger.error("安全过滤配置文件xss_security_config.xml解析异常", e);
        }catch (IOException e) {
            logger.error("安全过滤配置文件xss_security_config.xml加载异常", e);
        }
        logger.info("XSSSecurityManager init(FilterConfig config) end");
     ...
     * 
     * @param path 配置文件地址 eg
     *        C:/apache-tomcat-6.0.33/webapps/security_filter/WebRoot/config/xss/xss_security_config
     *        .xml
     * @return
     * @throws DocumentException
     */
    @SuppressWarnings("unchecked")
    public static boolean initConfig(Resource path) throws DocumentException,IOException {
        logger.info("XSSSecurityManager.initConfig(String path) begin");
        Element superElement = new SAXReader().read(path.getInputStream()).getRootElement();
        XSSSecurityConfig.IS_CHECK_HEADER =
                new Boolean(getEleValue(superElement, XSSSecurityCon.IS_CHECK_HEADER));
        XSSSecurityConfig.IS_CHECK_PARAMETER =
                new Boolean(getEleValue(superElement, XSSSecurityCon.IS_CHECK_PARAMETER));
        XSSSecurityConfig.IS_LOG = new Boolean(getEleValue(superElement, XSSSecurityCon.IS_LOG));
        XSSSecurityConfig.IS_CHAIN =
                new Boolean(getEleValue(superElement, XSSSecurityCon.IS_CHAIN));
